...
public class TrieSET implements Iterable<String> {
    ...
    public boolean addOneChar(char c){
    	Node curNode=nodeStack.peek();
    	if(curNode.next[c-'A']==null) return false;
    	nodeStack.push(curNode.next[c-'A']);
    	pathStack.push(c);
    	return true;
    }
    
    public boolean removeOneChar(){
    	if(nodeStack.size()>1){
    		nodeStack.pop();
    		pathStack.pop();
    		return true;
    ...
    public String pathToString(){
    	...
    	Iterator<Character> iter=pathStack.iterator();
    	while(iter.hasNext()){
    		sb.append(iter.next());
    	}
